...
@author: ltxu
@file: DeepFM_2.py
@time: 2019/7/24 上午8:55
@desc:  DeepFM的参数 重新划分train/test  最好的test_auc为0.7126  epoch 400左右
'''
...
optimizer = tf.train.AdamOptimizer(learning_rate=dfm_params['learning_rate'], beta1=0.9, beta2=0.999 ).minimize(loss)
# %%
# merged = tf.summary.merge()
saver = tf.train.Saver()
with tf.Session() as sess:
    writer = tf.summary.FileWriter(logdir='dfm_2_log', graph=sess.graph)
    sess.run(tf.global_variables_initializer())
    # sess.run(tf.local_variables_initializer())
    ckpt = tf.train.get_checkpoint_state(checkpoint_dir='dfm_2_models')
    ...
    for i in range(last_step+1,dfm_params['epoch']+1):
        _, loss_value, out_value = sess.run([optimizer, loss, out], feed_dict={feat_index: train_feature_index,
        ...
        if i%10==0:
            saver.save(sess, save_path='dfm_2_models/dfm', global_step=global_step)
