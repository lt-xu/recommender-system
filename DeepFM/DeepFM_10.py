...
@author: ltxu
@file: DeepFM_10.py
@time: 2019/7/24 下午5:27
@desc: 使用评分作为label计算rmse, 学习率为0.0001,le_reg为0.001   没有达到最好的结果且过拟合（trainloss降，testloss升），陷入了局部最优
'''
...
deep_dropout = tf.placeholder(dtype=np.float32, shape=[None], name='deep_dropout')
# %%
weights = dict()
...
if dfm_params['use_deep']:
    ...
    for i in range(num_layer):
        ...
        y_deep = tf.nn.dropout(y_deep, keep_prob=deep_dropout[i])
...
loss = tf.sqrt(tf.reduce_mean(tf.square(out-label))) #mse 作为 loss
...
loss_reg_value_ = tf.placeholder(dtype=np.float32, name='loss_reg_value_')
train_loss_reg_summary = tf.summary.scalar(name='train_loss_reg', tensor=loss_reg_value_)
rmse_value_ = tf.placeholder(dtype=np.float32, name='rmse_value_')
train_rmse_summary = tf.summary.scalar(name='train_rmse', tensor=rmse_value_)
test_rmse_summary = tf.summary.scalar(name='test_rmse', tensor=rmse_value_)
...
with tf.Session() as sess:
    ...
    ckpt = tf.train.get_checkpoint_state(checkpoint_dir=models_dir)
    ...
    for i in range(last_step+1,dfm_params['epoch']+1):
        sess.run(optimizer, feed_dict={feat_index: train_feature_index,
                                                               feat_value: train_feature_value,
                                                                deep_dropout:dfm_params['dropout_deep'],
        ...
        if i%10==0:
            train_loss_reg, mse_value= sess.run([loss_reg, loss], feed_dict={feat_index: train_feature_index,
                                                               feat_value: train_feature_value,
                                                            deep_dropout: dfm_params['dropout_deep'],
                                                                         label: y_train.reshape(-1, 1)})
            print('epoch{}, loss_reg: {}'.format(i, train_loss_reg))
            rmse_value = np.sqrt(mse_value)
            if i % 100 == 0:
                saver.save(sess, save_path=models_dir+'/dfm', global_step=global_step)

            train_loss_reg_summary_value, train_rmse_summary_value = sess.run([train_loss_reg_summary, train_rmse_summary],
                                                                              feed_dict={loss_reg_value_:train_loss_reg, rmse_value_:rmse_value})
            writer.add_summary(train_loss_reg_summary_value, global_step=i)
            writer.add_summary(train_rmse_summary_value, global_step=i)

            mse_value = sess.run(loss, feed_dict={feat_index:test_feature_index,
                                                                     feat_value: test_feature_value,
                                                                     deep_dropout: [1.0,1.0,1.0],
                                                                     label:y_test.reshape(-1,1)})
            rmse_value = np.sqrt(mse_value)
            test_rmse_summary_value = sess.run(test_rmse_summary, feed_dict={rmse_value_:rmse_value})
            writer.add_summary(test_rmse_summary_value, global_step= i)
