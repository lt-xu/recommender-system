...
with tf.Session() as sess:
    writer = tf.summary.FileWriter(logdir='dfm_log', graph=sess.graph)
    sess.run(tf.global_variables_initializer())
    # sess.run(tf.local_variables_initializer())
    ckpt = tf.train.get_checkpoint_state(checkpoint_dir='dfm_models')
    ...
    for i in range(last_step+1,dfm_params['epoch']+1):
        ...
        if i%10==0:
            saver.save(sess, save_path='dfm_models/dfm', global_step=global_step)
