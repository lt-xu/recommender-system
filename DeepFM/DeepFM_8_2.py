...
@desc: # 参数与DeepFM_8相同，更换train test 最好的test_auc 为 0.6712 2k次迭代
'''
...
logdir = 'dfm_8_2_log'
models_dir = 'dfm_8_2_models'
data_merged = pd.read_csv('data_merged.csv')
cols = ['user','item','rating','timestamp']

train = pd.read_csv('ml-100k/ua.base', sep='\t',names=cols)
train = train.merge(right=data_merged, how='left', left_on=['user','item'], right_on=['user', 'item'])
y_train = train.like.values.reshape(-1,1)
train.drop(['user', 'item','rating_x','rating_y','timestamp_x', 'timestamp_y',  'datetime', 'like'], axis=1, inplace = True)

test = pd.read_csv('ml-100k/ua.test', sep='\t',names=cols)
test = test.merge(right=data_merged, how='left', left_on=['user','item'], right_on=['user', 'item'])
y_test = test.like.values.reshape(-1,1)
test.drop(['user', 'item','rating_x','rating_y','timestamp_x', 'timestamp_y',  'datetime', 'like'], axis=1, inplace = True)
...
with tf.Session() as sess:
    ...
    for i in range(last_step+1,dfm_params['epoch']+1):
        ...
        if i%10==0:
            ...
            loss_value, out_value = sess.run([loss, out], feed_dict={feat_index:test_feature_index,
